[x indices]=get_image_data_v3('s2.jpg');
% x=importdata('x.txt');
...
show_image_v3(x,idx,uidx,'s2.jpg',indices)
%mkplot(x,idx);
...
function [X all_indices]=get_image_data_v3(filename);
    ...
    X=[];
    I1=double(imread('s2_entro.jpg'));%gaussian blur
    I2=double(imread('s2_med.jpg'));%median filtering
    I3=double(imread('s2_gauss.jpg'));%entropy
    I_mask=double(imread('s2_mask.jpg'));
